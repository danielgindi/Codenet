...
public class WuColorQuantizer : BaseColorQuantizer
{
    ...
    {
        Int32 indexRed = (color.Red >> 3) + 1;
        Int32 indexGreen = (color.Green >> 3) + 1;
        Int32 indexBlue = (color.Blue >> 3) + 1;

        weights[indexRed, indexGreen, indexBlue]++;
        momentsRed[indexRed, indexGreen, indexBlue] += color.Red;
        momentsGreen[indexRed, indexGreen, indexBlue] += color.Green;
        momentsBlue[indexRed, indexGreen, indexBlue] += color.Blue;
        moments[indexRed, indexGreen, indexBlue] += table[color.Red] + table[color.Green] + table[color.Blue];
    ...
    /// </summary>
    protected override List<NeatColor> OnGetPalette(Int32 colorCount)
    {
        ...
        for (Int32 index = 0; index < imageSize; index++)
        {
            NeatColor color = new NeatColor(pixels[index]);
            ...
            for (Int32 lookup = 0; lookup < colorCount; lookup++)
            {
                ...
                Int32 deltaRed = color.Red - foundRed;
                Int32 deltaGreen = color.Green - foundGreen;
                Int32 deltaBlue = color.Blue - foundBlue;
            ...
            reds[bestMatch] += color.Red;
            greens[bestMatch] += color.Green;
            blues[bestMatch] += color.Blue;
            sums[bestMatch]++;

            indices[index] = bestMatch;
        }

        List<NeatColor> result = new List<NeatColor>();
        ...
        for (Int32 paletteIndex = 0; paletteIndex < colorCount; paletteIndex++)
        {
            ...
            NeatColor color = NeatColor.FromARGB(255, (byte)reds[paletteIndex], (byte)greens[paletteIndex], (byte)blues[paletteIndex]);
            result.Add(color);
        }
