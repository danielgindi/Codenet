...
namespace Codenet.Drawing.Quantizers.NeuQuant;
...
public class NeuralColorQuantizer : BaseColorQuantizer
{
    ...
    private void LearnSampleColor(NeatColor color)
    {
        Int32 red = color.Red << NetworkBiasShift;
        Int32 green = color.Green << NetworkBiasShift;
        Int32 blue = color.Blue << NetworkBiasShift;
    ...
    protected override List<NeatColor> OnGetPalette(Int32 colorCount)
    {
        ...
        for (Int32 index = 0; index < NetworkSize; index++)
        {
            ...
            NeatColor color = NeatColor.FromARGB(255, (byte)red, (byte)green, (byte)blue);
            result.Add(color);
        }
    ...
    protected override void OnGetPaletteIndex(NeatColor color, UInt32 key, Int32 x, Int32 y, out Int32 paletteIndex)
    {
        ...
        Int32 increaseIndex = networkIndexLookup[color.Green];
        ...
        while (increaseIndex < NetworkSize || decreaseIndex >= 0)
        {
            if (increaseIndex < NetworkSize)
            {
                ...
                // add green delta
                Int32 deltaG = neuron[1] - color.Green;
                ...
                else
                {
                    ...
                    // add blue delta
                    Int32 deltaB = neuron[0] - color.Blue;
                    ...
                    {
                        // add red delta
                        Int32 deltaR = neuron[2] - color.Red;
            ...
            if (decreaseIndex >= 0)
            {
                ...
                // add green delta
                Int32 deltaG = color.Green - neuron[1];
                if (deltaG < 0) deltaG = -deltaG;
                Int32 distance = deltaG;
                ...
                else
                {
                    ...
                    // add blue delta
                    Int32 deltaBlue = neuron[0] - color.Blue;
                    if (deltaBlue < 0) deltaBlue = -deltaBlue;
                    distance += deltaBlue;
                    ...
                    {
                        // add red delta
                        Int32 deltaRed = neuron[2] - color.Red;
                        if (deltaRed < 0) deltaRed = -deltaRed;
                        distance += deltaRed;
