...
using System.IO;
using Codenet.Drawing.ImageProcessing.Quantizers.NeuQuant;
    ...
    public class GifEncoder : IDisposable
    {
        ...
        protected void AnalyzePixels()
        {
            ...
            using (Image quantizedImage = new Bitmap(_FrameImage.Width, _FrameImage.Height, PixelFormatUtility.GetFormatByColorCount(256)))
            {
                ...
                using (var quantizedImageBuffer = new ImageBuffer(quantizedImage, ImageLockMode.ReadWrite))
                {
                    using (var sourceImageBuffer = new ImageBuffer(_FrameImage, ImageLockMode.ReadOnly))
                    {
                        int transparentPixelX = -1, transparentPixelY = -1;
                        var hasTransparentColor = _TransparentColor != Color.Empty && sourceImageBuffer.ScanForColor(_TransparentColor, out transparentPixelX, out transparentPixelY);
                        ...
                        var pixel = new Pixel(quantizedImageBuffer);
                        j = 0;
